...
/// DATABASE_POOL
pub static DATABASE_POOL: OnceCell<MySqlPool> = OnceCell::new();

pub async fn init_db_pool() -> Result<Option<&'static MySqlPool>, Box<dyn std::error::Error>>{

    // 获取配置的环境变量
    if let Ok(uri) = &env::var("MYSQL_URL") {    
        println!("uri: {}", uri);
...
#[allow(dead_code)]
pub fn db_pool() -> Option<&'static MySqlPool> {
    DATABASE_POOL.get()
}
...
pub trait DbPool {
    fn get_pool() -> Option<&'static MySqlPool>; 
}
